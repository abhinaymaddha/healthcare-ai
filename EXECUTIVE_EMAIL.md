...
**What makes this approach operationally viable.** We use a two-tier LLM strategy: a small, cost-efficient model handles intent classification, clarification, medication extraction, and summarization; a medium-size model generates clinical responses where quality, empathy, and safety nuance matter most. In our testing, we used Claude Haiku 4.5 (small LLM) and Claude Sonnet 4.6 (medium-size LLM) via OpenRouter, at approximately $0.005–0.008 per conversation. Emergency escalation runs entirely on local rule-based logic — no LLM call, no latency, no cost. The architecture is horizontally scalable and model-agnostic; switching to a different AI provider or model tier requires a single configuration change.
